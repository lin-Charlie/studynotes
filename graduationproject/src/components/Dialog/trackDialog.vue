      ...
      <el-collapse v-model="activeNames">
        <el-collapse-item title="审批进度" name="1" class="approval_progress">
          ...
          <el-row> 
            ...
            <el-col :span="12" v-if="options.details.state==1">{{`${options.details.executePerson} 终止于 ${options.details.completeTime}`}}</el-col>
            <el-col :span="12" v-if="options.details.state==2">{{`${options.details.executePerson} 完成于 ${options.details.completeTime}`}}</el-col>
          </el-row>
        </el-collapse-item>
        ...
        <el-collapse-item title="执行过程" name="3">
          ...
          <el-row v-if="options.details.list!==[]"> 
              <el-col :span="24">
                <div
                  class="block"
                  v-for="file in options.details.list"
                  :key="file.id"
                >
                  <el-image
                    v-if="(/\.(?:jpg|jpeg)$/).test(file.prefix)"
              ...
              <div
                class="block"
                v-for="(file, index) in options.details.list"
                :key="index"
              >
                <span style="diaplay:inline-block" v-if="(/\.(?:doc)$/).test(file.prefix)" 
                @click="download(file)">{{file.fileName}}</span>
              </div>
          </el-row>
        </el-collapse-item>
      </el-collapse>
