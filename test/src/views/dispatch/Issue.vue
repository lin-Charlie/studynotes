...
export default {
  ...
  methods: {
    ...
    // 分类指令查询
    async serchDis(selectVal,createTime,planTime) {
      // let {data} = await this.$axios({
      //   url:`/management/pocommand/findWait/${this.pageParams.pageNum}/${this.pageParams.pageSize}`,
      //   method:"get"
      // })
      // if(data){
    ...
    trackIssue(row) {
      ...
      // 文件或图片列表数据
      this.trackOptions.details.list = []
    },
    endIssue(row) {
    ...
    async getInstructData(page, size) {
      ...
      size = size||10;
      let {data} = await this.$axios({
            url:`/management/pocommand/findPoCommand/${page}/${size}`,
            method:"get"
      })
      // 有无数据
      if(data){
